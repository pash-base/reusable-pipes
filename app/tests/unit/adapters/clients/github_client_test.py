...
def test_should_run_git_commands_when_commit_and_push_is_called(mocker):
    ...
    mocker.patch("builtins.open", mocker.mock_open(read_data="tag: old"))
    ...
    assert calls[0].args[0] == ["git", "checkout", "--", "app/_environments/dev/values-dev.yaml"]
    assert calls[1].args[0] == ["git", "checkout", "develop"]
    assert calls[2].args[0] == ["git", "pull", "--rebase", "--autostash", "origin", "develop"]
    assert calls[3].args[0] == ["git", "add", "app/_environments/dev/values-dev.yaml"]
    assert calls[4].args[0] == ["git", "commit", "-m", "chore(gitops): update tag"]
    assert calls[5].args[0] == ["git", "push", "origin", "develop"]
